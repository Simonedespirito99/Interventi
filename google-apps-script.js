...
// 4. Modifica CONFIG.DRIVE_FOLDER_ID con l'ID della tua cartella Drive

// 🔧 CONFIGURAZIONE PRINCIPALE - MODIFICA QUESTI VALORI
const CONFIG = {
  SHEET_ID: 'INSERISCI_QUI_IL_TUO_GOOGLE_SHEET_ID', // ⚠️ MODIFICA QUESTO!
  DRIVE_FOLDER_ID: 'INSERISCI_QUI_ID_CARTELLA_DRIVE', // ⚠️ MODIFICA QUESTO!
...
function uploadImageToDrive(base64Data, fileName, identificativo, tipoIntervento, buonoLavoro) {
  try {
    ...
    // STEP 1: Trova la cartella specifica usando l'ID configurato
    let mainFolder;
    try {
      if (CONFIG.DRIVE_FOLDER_ID && CONFIG.DRIVE_FOLDER_ID !== 'INSERISCI_QUI_ID_CARTELLA_DRIVE') {
        ...
        throw new Error('ID cartella Drive non configurato');
      }
    } catch (folderError) {
    ...
    if (tipoIntervento === 'mud') {
      // Per MUD usa il buono lavoro (più affidabile del parsing dell'identificativo)
    ...
    console.log('🎯 Logica usata:', tipoIntervento === 'mud' ? 'MUD → usa Buono Lavoro' : 'Ordinario → usa Buono Lavoro');
...
function testScriptDueFogli() {
  ...
  if (!CONFIG.DRIVE_FOLDER_ID || CONFIG.DRIVE_FOLDER_ID === 'INSERISCI_QUI_ID_CARTELLA_DRIVE') {
    console.error('❌ ERRORE: Devi configurare CONFIG.DRIVE_FOLDER_ID!');
    console.log('💡 Modifica la variabile CONFIG.DRIVE_FOLDER_ID all\'inizio del codice');
    return { error: 'Drive Folder ID non configurato' };
  }
  
  // Test configurazione
  console.log('📊 Configurazione attuale:');
  console.log('  SHEET_ID:', CONFIG.SHEET_ID);
  console.log('  DRIVE_FOLDER_ID:', CONFIG.DRIVE_FOLDER_ID);
